export interface EquipmentStats {
  ...
  autoClickRate?: number;    // For ALL equippable items - auto-clicks per second (stacks across items)
                            // Strategy: Common 0.5/s, Rare 1.0/s, Epic 1.5-2.0/s, Legendary 2.5-4.0/s
...
export interface LootItem {
  ...
  cleansesDebuffs?: boolean; // Optional flag for consumables that remove all DoT effects
  removesMonsterShield?: boolean; // Optional flag for consumables that instantly break monster shields
}

export interface ConsumableBuffData {
  buffType: 'shield' | 'damage_boost' | 'damage_mult' | 'damage_reduction' | 'crit_boost' | 'attack_speed' | 'cooldown_reduction' | 'coin_boost' | 'xp_boost' | 'heal_boost' | 'fire_resistance' | 'poison_resistance' | 'bleed_resistance';
...
const COMMON_LOOT: LootItem[] = [
  ...
  { lootId: 'common_dagger', name: 'Rusty Dagger', icon: '🗡️', description: 'A worn but functional blade', rarity: 'common', type: 'weapon', equipmentStats: { damageBonus: 1, autoClickRate: 0.5 } },
...
const RARE_LOOT: LootItem[] = [
  ...
  { lootId: 'rare_amulet', name: 'Mystic Amulet', icon: '📿', description: 'Enhances magical abilities', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 10, autoClickRate: 0.5 } },
  { lootId: 'rare_key', name: 'Ancient Key', icon: '🗝️', description: 'Opens mysterious locks', rarity: 'rare', type: 'artifact', equipmentStats: { coinBonus: 15, autoClickRate: 0.5 } },
  { lootId: 'rare_map', name: 'Treasure Map', icon: '🗺️', description: 'Leads to hidden riches', rarity: 'rare', type: 'artifact', equipmentStats: { coinBonus: 20, autoClickRate: 0.5 } },
  // Heal-focused items (sustainability builds)
  { lootId: 'regeneration_pendant', name: 'Regeneration Pendant', icon: '💚', description: 'Enhances natural healing (+4% heal)', rarity: 'rare', type: 'artifact', equipmentStats: { healBonus: 4, autoClickRate: 0.5 } },
  { lootId: 'ring_of_vitality', name: 'Ring of Vitality', icon: '💍', description: 'Grants enhanced recovery (+5% heal)', rarity: 'rare', type: 'artifact', equipmentStats: { healBonus: 5, maxHpBonus: 5, autoClickRate: 0.5 } },
  ...
  // NEW: Strategic Depth consumables (Tank builds)
  { lootId: 'iron_skin_potion', name: 'Iron Skin Potion', icon: '🛡️', description: 'Grants 30% damage reduction for 30s', rarity: 'rare', type: 'consumable', cooldown: 20, buffData: { buffType: 'damage_reduction', buffValue: 30, duration: 30 } },
  { lootId: 'cleanse_potion', name: 'Cleanse Potion', icon: '💫', description: 'Removes all DoT effects instantly', rarity: 'rare', type: 'consumable', cooldown: 15, cleansesDebuffs: true },
  // NEW: Strategic Depth consumables (Offensive builds)
  { lootId: 'battle_elixir', name: 'Battle Elixir', icon: '⚔️', description: 'Grants +40% damage for 25s', rarity: 'rare', type: 'consumable', cooldown: 25, buffData: { buffType: 'damage_mult', buffValue: 40, duration: 25 } },
  { lootId: 'crit_potion', name: 'Critical Strike Potion', icon: '🎯', description: 'Grants +35% crit chance for 20s', rarity: 'rare', type: 'consumable', cooldown: 22, buffData: { buffType: 'crit_boost', buffValue: 35, duration: 20 } },
];
...
const GOBLIN_ORC_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'tribal_mask', name: 'Tribal Mask', icon: '🎭', description: 'A crude ceremonial mask', rarity: 'rare', type: 'artifact', equipmentStats: { critChance: 5, autoClickRate: 1.0 } },
...
const TROLL_GHOST_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'silver_sword', name: 'Silver Sword', icon: '⚔️', description: 'Effective against the undead', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 5, autoClickRate: 1.0 } },
  { lootId: 'spirit_crystal', name: 'Spirit Crystal', icon: '💎', description: 'Contains trapped souls', rarity: 'rare', type: 'artifact', equipmentStats: { maxHpBonus: 10, autoClickRate: 1.0 } },
  { lootId: 'enchanted_ring', name: 'Enchanted Ring', icon: '💍', description: 'Hums with magical energy', rarity: 'rare', type: 'artifact', equipmentStats: { coinBonus: 25, critChance: 3, autoClickRate: 1.0 } },
  { lootId: 'mana_potion', name: 'Mana Potion', icon: '⚗️', description: 'Restores 25 HP', rarity: 'rare', type: 'consumable', cooldown: 8, healing: 25 },
  { lootId: 'spectral_cloak', name: 'Spectral Cloak', icon: '🧥', description: 'Grants temporary invisibility', rarity: 'epic', type: 'armor', equipmentStats: { defense: 10, maxHpBonus: 15, autoClickRate: 1.5 } },
...
const DRAGON_VAMPIRE_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'flame_sword', name: 'Flame Sword', icon: '🔥', description: 'Blade wreathed in eternal fire', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 4, critChance: 10, autoClickRate: 1.8 } },
  { lootId: 'blood_chalice', name: 'Blood Chalice', icon: '🏆', description: 'An ancient vampiric relic', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 25, coinBonus: 30, autoClickRate: 1.5 } },
  { lootId: 'dragon_armor', name: 'Dragon Scale Armor', icon: '🛡️', description: 'Nearly impenetrable defense - spiked scales reflect damage', rarity: 'epic', type: 'armor', equipmentStats: { defense: 15, maxHpBonus: 30, thorns: 12, autoClickRate: 1.5 } },
  ...
  { lootId: 'crimson_crown', name: 'Crimson Crown', icon: '👑', description: 'Symbol of vampiric royalty', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 50, critChance: 15, coinBonus: 50, autoClickRate: 3.0 } },
  // NEW: Strategic Depth consumables (Epic tier)
  { lootId: 'stone_skin_elixir', name: 'Stone Skin Elixir', icon: '🗿', description: 'Grants 50% damage reduction for 30s - Ultimate protection', rarity: 'epic', type: 'consumable', cooldown: 30, buffData: { buffType: 'damage_reduction', buffValue: 50, duration: 30 } },
  { lootId: 'berserker_brew', name: 'Berserker Brew', icon: '🍺', description: 'Grants +60% damage for 20s - Pure offense', rarity: 'epic', type: 'consumable', cooldown: 35, buffData: { buffType: 'damage_mult', buffValue: 60, duration: 20 } },
  { lootId: 'acid_vial', name: 'Acid Vial', icon: '🧪', description: 'Instantly removes monster shield', rarity: 'epic', type: 'consumable', cooldown: 25, removesMonsterShield: true },
  { lootId: 'frost_vial', name: 'Frost Vial', icon: '❄️', description: 'Slows monster attacks by 40% for 15s', rarity: 'epic', type: 'consumable', cooldown: 20, buffData: { buffType: 'attack_speed', buffValue: 40, duration: 15 } },
...
const DEMON_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'soul_stone', name: 'Soul Stone', icon: '💠', description: 'Contains thousands of trapped souls', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 100, critChance: 20, attackSpeed: 15, autoClickRate: 3.5 } },
  { lootId: 'infernal_blade', name: 'Infernal Blade', icon: '🗡️', description: 'Forged in the fires of hell - pure offensive power', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 8, critChance: 16, autoClickRate: 3.8 } },
  { lootId: 'demon_eye', name: 'Demon Eye', icon: '👁️', description: 'Sees through all illusions', rarity: 'legendary', type: 'material' },
  { lootId: 'hellfire_staff', name: 'Hellfire Staff', icon: '🪄', description: 'Commands the flames of perdition - devastating magical assault', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 8, critChance: 14, autoClickRate: 3.8 } },
  { lootId: 'void_armor', name: 'Void Armor', icon: '🛡️', description: 'Forged from pure darkness - devours pain as sustenance', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 25, maxHpBonus: 75, defensiveLifesteal: 5, autoClickRate: 2.5 } },
  { lootId: 'demonic_tome', name: 'Demonic Tome', icon: '📖', description: 'Contains forbidden knowledge', rarity: 'legendary', type: 'artifact', equipmentStats: { critChance: 25, attackSpeed: 15, autoClickRate: 3.5 } },
  { lootId: 'chaos_orb', name: 'Chaos Orb', icon: '🔮', description: 'Reality bends around it', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 5, maxHpBonus: 50, coinBonus: 100, autoClickRate: 3.0 } },
  { lootId: 'dark_halo', name: 'Dark Halo', icon: '⭕', description: 'Corrupts all who wear it', rarity: 'legendary', type: 'artifact', equipmentStats: { critChance: 30, defense: 10, autoClickRate: 3.5 } },
...
const FOREST_WOLF_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'leather_vest', name: 'Leather Vest', icon: '🧥', description: 'Basic protective gear', rarity: 'common', type: 'armor', equipmentStats: { defense: 5, autoClickRate: 0.5 } },
  { lootId: 'hunter_dagger', name: 'Hunter\'s Dagger', icon: '🗡️', description: 'A reliable hunting blade', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 3, autoClickRate: 1.0 } },
  { lootId: 'stolen_coin', name: 'Stolen Gold', icon: '🪙', description: 'Ill-gotten gains', rarity: 'common', type: 'material' },
  { lootId: 'bandana', name: 'Bandit Bandana', icon: '🏴', description: 'Worn by forest outlaws', rarity: 'rare', type: 'artifact', equipmentStats: { coinBonus: 10, autoClickRate: 1.0 } },
...
const WILD_BOAR_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'heavy_armor', name: 'Heavy Leather Armor', icon: '🛡️', description: 'Reinforced protection', rarity: 'rare', type: 'armor', equipmentStats: { defense: 10, maxHpBonus: 10, autoClickRate: 1.0 } },
  { lootId: 'charging_boots', name: 'Charging Boots', icon: '👢', description: 'Enables quick strikes', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 5, autoClickRate: 1.0 } },
...
const FOREST_SPRITE_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'nature_staff', name: 'Nature Staff', icon: '🪄', description: 'Channel nature\'s power', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 5, autoClickRate: 1.0 } },
  { lootId: 'sprite_ring', name: 'Sprite Ring', icon: '💍', description: 'Enhances agility', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 8, autoClickRate: 1.0 } },
...
const TREANT_GUARDIAN_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'guardian_armor', name: 'Guardian\'s Bark Armor', icon: '🛡️', description: 'Nature\'s protection', rarity: 'epic', type: 'armor', equipmentStats: { defense: 15, maxHpBonus: 20, autoClickRate: 1.5 } },
  { lootId: 'nature_blade', name: 'Nature\'s Wrath', icon: '🗡️', description: 'Living weapon', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 5, critChance: 8, autoClickRate: 1.8 } },
  { lootId: 'forest_crown', name: 'Crown of the Forest', icon: '👑', description: 'Blessed by the ancients', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 15, coinBonus: 10, autoClickRate: 1.5 } },
  // Heal-focused items (nature healing theme)
  { lootId: 'medics_robe', name: 'Medic\'s Robe', icon: '🧥', description: 'Blessed by forest healers (+6% heal)', rarity: 'epic', type: 'armor', equipmentStats: { defense: 8, maxHpBonus: 12, healBonus: 6, autoClickRate: 1.5 } },
  { lootId: 'amulet_of_life', name: 'Amulet of Life', icon: '🌿', description: 'Channels nature\'s vitality (+8% heal)', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 15, healBonus: 8, autoClickRate: 1.5 } },
...
const DIRE_WOLF_ALPHA_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'alpha_blade', name: 'Alpha Fang Sword', icon: '⚔️', description: 'Forged from the alpha\'s fang', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 5, critChance: 11, attackSpeed: 8, autoClickRate: 2.0 } },
  { lootId: 'alpha_armor', name: 'Alpha Pelt Armor', icon: '🛡️', description: 'Legendary wolf hide protection', rarity: 'epic', type: 'armor', equipmentStats: { defense: 14, maxHpBonus: 18, autoClickRate: 1.5 } },
  { lootId: 'pack_totem', name: 'Pack Leader\'s Totem', icon: '🗿', description: 'Commands respect from all beasts', rarity: 'epic', type: 'artifact', equipmentStats: { damageBonus: 3, critChance: 9, coinBonus: 15, autoClickRate: 1.8 } },
];
...
const ANCIENT_ENT_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'natures_judgment', name: 'Nature\'s Judgment', icon: '🪄', description: 'Ancient staff of the forest - balanced power and utility', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 7, critChance: 13, attackSpeed: 8, autoClickRate: 3.5 } },
  { lootId: 'ancient_bark_plate', name: 'Ancient Bark Plate', icon: '🛡️', description: 'Millennium-old protection', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 24, maxHpBonus: 55, autoClickRate: 2.5 } },
  { lootId: 'heart_of_forest', name: 'Heart of the Forest', icon: '💚', description: 'Essence of all nature', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 45, critChance: 14, coinBonus: 65, autoClickRate: 3.0 } },
  // Legendary heal-focused items (ultimate sustainability)
  { lootId: 'heart_crystal', name: 'Heart Crystal', icon: '💎', description: 'Ancient crystal pulsing with life (+12% heal)', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 40, healBonus: 12, autoClickRate: 2.8 } },
  { lootId: 'lifebringers_crown', name: 'Lifebringer\'s Crown', icon: '👑', description: 'Crown of the eternal healer (+15% heal)', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 50, healBonus: 15, defense: 10, autoClickRate: 3.2 } },
...
const SAND_SCORPION_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'desert_cloth', name: 'Desert Wraps', icon: '🧣', description: 'Protects from heat and venom', rarity: 'common', type: 'armor', equipmentStats: { defense: 6, autoClickRate: 0.5 } },
  { lootId: 'venom_dagger', name: 'Venom Blade', icon: '🗡️', description: 'Coated with poison', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, autoClickRate: 1.0 } },
];
...
const DESERT_VIPER_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'speed_boots', name: 'Sandstorm Boots', icon: '👟', description: 'Swift as the desert wind', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 8, autoClickRate: 1.0 } },
...
const FIRE_ELEMENTAL_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'flame_sword_desert', name: 'Flame Sword', icon: '🗡️', description: 'Blade wreathed in fire', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 4, critChance: 10, autoClickRate: 1.8 } },
  { lootId: 'fire_resist_charm', name: 'Flame Ward Amulet', icon: '📿', description: 'Protects from fire (+50% fire resist)', rarity: 'rare', type: 'artifact', equipmentStats: { maxHpBonus: 5, fireResistance: 50, autoClickRate: 1.0 } },
...
const SAND_DJINN_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'djinn_scimitar', name: 'Djinn\'s Scimitar', icon: '⚔️', description: 'Curved blade of legend', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 5, critChance: 12, attackSpeed: 5, autoClickRate: 2.0 } },
  { lootId: 'sand_armor', name: 'Djinn\'s Sand Armor', icon: '🛡️', description: 'Flows like sand', rarity: 'epic', type: 'armor', equipmentStats: { defense: 12, maxHpBonus: 15, autoClickRate: 1.5 } },
  { lootId: 'mirage_cloak', name: 'Mirage Cloak', icon: '🧥', description: 'Bends light around wearer', rarity: 'epic', type: 'artifact', equipmentStats: { coinBonus: 15, autoClickRate: 1.5 } },
...
const SANDSTONE_GOLEM_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'golem_hammer', name: 'Golem\'s Wrath Hammer', icon: '🔨', description: 'Crushes with ancient power', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 10, maxHpBonus: 15, autoClickRate: 1.8 } },
  { lootId: 'sandstone_armor', name: 'Sandstone Plate', icon: '🛡️', description: 'Impenetrable stone armor', rarity: 'epic', type: 'armor', equipmentStats: { defense: 16, maxHpBonus: 25, autoClickRate: 1.5 } },
  { lootId: 'desert_stone_amulet', name: 'Desert Stone Amulet', icon: '📿', description: 'Grants earth\'s endurance', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 20, defense: 8, autoClickRate: 1.5 } },
];
...
const DESERT_PHOENIX_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'phoenix_talon', name: 'Phoenix Talon Blade', icon: '⚔️', description: 'Forged from eternal flames - sustain-focused blade for tank builds', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 10, attackSpeed: 15, defense: 10, maxHpBonus: 25, lifesteal: 4, autoClickRate: 3.0 } },
  { lootId: 'phoenix_plate', name: 'Phoenix Flame Plate', icon: '🛡️', description: 'Armor of rebirth', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 26, maxHpBonus: 65, autoClickRate: 2.5 } },
  { lootId: 'eternal_flame_orb', name: 'Eternal Flame Orb', icon: '🔮', description: 'Contains the phoenix\'s power', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 5, maxHpBonus: 50, critChance: 18, coinBonus: 75, autoClickRate: 3.2 } },
...
const CORAL_CRAB_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'shell_armor', name: 'Shell Armor', icon: '🛡️', description: 'Reinforced with crab shell', rarity: 'common', type: 'armor', equipmentStats: { defense: 7, maxHpBonus: 5, autoClickRate: 0.5 } },
  { lootId: 'coral_dagger', name: 'Coral Dagger', icon: '🗡️', description: 'Sharp as a reef', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 3, autoClickRate: 1.0 } },
];
...
const GIANT_JELLYFISH_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'jelly_armor', name: 'Jellyfish Membrane', icon: '🛡️', description: 'Flexible protection', rarity: 'common', type: 'armor', equipmentStats: { defense: 5, maxHpBonus: 8, autoClickRate: 0.5 } },
  { lootId: 'stinger_whip', name: 'Stinger Whip', icon: '🪢', description: 'Paralyzing strikes', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 1, attackSpeed: 8, autoClickRate: 1.0 } },
];
...
const FROST_SHARK_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'frost_blade', name: 'Frost Blade', icon: '🗡️', description: 'Freezes enemies', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 3, critChance: 7, autoClickRate: 1.0 } },
  { lootId: 'ice_armor', name: 'Frost Guard Armor', icon: '🛡️', description: 'Chills attackers', rarity: 'rare', type: 'armor', equipmentStats: { defense: 11, maxHpBonus: 12, autoClickRate: 1.0 } },
  { lootId: 'frozen_heart', name: 'Frozen Heart', icon: '💙', description: 'Cold to the touch', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 10, critChance: 5, autoClickRate: 1.5 } },
];
...
const ELECTRIC_EEL_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'shock_dagger', name: 'Shock Dagger', icon: '🗡️', description: 'Electrifies on contact', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 8, autoClickRate: 1.0 } },
  { lootId: 'storm_amulet', name: 'Storm Amulet', icon: '📿', description: 'Crackles with energy', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 10, coinBonus: 12, autoClickRate: 1.0 } },
...
const SEA_SERPENT_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'storm_blade', name: 'Storm Blade', icon: '⚔️', description: 'Channels lightning', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 5, critChance: 10, attackSpeed: 8, autoClickRate: 2.0 } },
  { lootId: 'tidal_armor', name: 'Tidal Scale Armor', icon: '🛡️', description: 'Flows like water', rarity: 'epic', type: 'armor', equipmentStats: { defense: 14, maxHpBonus: 18, autoClickRate: 1.5 } },
  { lootId: 'ocean_crown', name: 'Crown of the Deep', icon: '👑', description: 'Commands the seas', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 20, coinBonus: 15, autoClickRate: 1.5 } },
];
...
const KRAKEN_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'tentacle_whip', name: 'Kraken\'s Whip', icon: '🪢', description: 'Crushes with tentacle power', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 11, maxHpBonus: 12, autoClickRate: 1.8 } },
  { lootId: 'kraken_armor', name: 'Kraken Scale Armor', icon: '🛡️', description: 'Forged from kraken hide', rarity: 'epic', type: 'armor', equipmentStats: { defense: 16, maxHpBonus: 22, autoClickRate: 1.5 } },
  { lootId: 'abyssal_amulet', name: 'Abyssal Amulet', icon: '📿', description: 'Commands the deep ocean', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 18, critChance: 8, coinBonus: 18, autoClickRate: 1.5 } },
];
...
const LEVIATHAN_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'trident', name: 'Poseidon\'s Trident', icon: '🔱', description: 'Legendary three-pronged spear - balanced power and utility', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 13, attackSpeed: 10, autoClickRate: 3.5 } },
  { lootId: 'leviathan_armor', name: 'Leviathan Plate', icon: '🛡️', description: 'Unbreakable defense', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 22, maxHpBonus: 50, autoClickRate: 2.5 } },
  { lootId: 'tidal_orb', name: 'Tidal Orb', icon: '🔮', description: 'Controls water itself', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 40, critChance: 15, coinBonus: 60, autoClickRate: 3.0 } },
...
const LAVA_SALAMANDER_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'flame_cloak', name: 'Flame Cloak', icon: '🧥', description: 'Resists fire (+30% fire resist)', rarity: 'common', type: 'armor', equipmentStats: { defense: 6, fireResistance: 30, autoClickRate: 0.5 } },
  { lootId: 'molten_dagger', name: 'Molten Dagger', icon: '🗡️', description: 'Drips with lava', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 4, autoClickRate: 1.0 } },
];
...
const FIRE_BAT_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'ember_blade', name: 'Ember Blade', icon: '🗡️', description: 'Burns with residual heat', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 5, autoClickRate: 1.0 } },
  { lootId: 'wing_boots', name: 'Wing Boots', icon: '👢', description: 'Grants extra speed', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 7, autoClickRate: 1.0 } },
];
...
const MAGMA_GOLEM_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'magma_hammer', name: 'Magma Hammer', icon: '🔨', description: 'Crushes with fiery force', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 4, critChance: 6, autoClickRate: 1.0 } },
  { lootId: 'obsidian_armor', name: 'Obsidian Plate', icon: '🛡️', description: 'Volcanic glass armor', rarity: 'rare', type: 'armor', equipmentStats: { defense: 12, maxHpBonus: 10, autoClickRate: 1.0 } },
...
const INFERNO_IMP_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'hellfire_dagger', name: 'Hellfire Dagger', icon: '🗡️', description: 'Cursed flames', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 3, critChance: 9, autoClickRate: 1.0 } },
  { lootId: 'inferno_ring', name: 'Ring of Inferno', icon: '💍', description: 'Enhances fire attacks', rarity: 'rare', type: 'artifact', equipmentStats: { damageBonus: 2, critChance: 8, autoClickRate: 1.0 } },
  { lootId: 'brimstone_armor', name: 'Brimstone Armor', icon: '🛡️', description: 'Demonic protection', rarity: 'rare', type: 'armor', equipmentStats: { defense: 10, maxHpBonus: 8, autoClickRate: 1.0 } },
];
...
const FIRE_DRAKE_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'dragonfire_sword', name: 'Dragonfire Sword', icon: '⚔️', description: 'Blazing with dragonfire', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 12, attackSpeed: 6, autoClickRate: 2.0 } },
  { lootId: 'drake_armor', name: 'Drake Scale Armor', icon: '🛡️', description: 'Forged from drake scales', rarity: 'epic', type: 'armor', equipmentStats: { defense: 15, maxHpBonus: 22, autoClickRate: 1.5 } },
  { lootId: 'flame_pendant', name: 'Flame Pendant', icon: '📿', description: 'Burns eternally', rarity: 'epic', type: 'artifact', equipmentStats: { critChance: 10, coinBonus: 18, autoClickRate: 1.5 } },
];
...
const VOLCANIC_TITAN_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'titan_fist', name: 'Titan Fist Gauntlet', icon: '✊', description: 'Strikes with volcanic fury', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 7, critChance: 10, maxHpBonus: 15, autoClickRate: 1.8 } },
  { lootId: 'molten_plate', name: 'Molten Titan Plate', icon: '🛡️', description: 'Volcanic armor of legends', rarity: 'epic', type: 'armor', equipmentStats: { defense: 17, maxHpBonus: 28, autoClickRate: 1.5 } },
  { lootId: 'eruption_ring', name: 'Ring of Eruption', icon: '💍', description: 'Harnesses volcanic power', rarity: 'epic', type: 'artifact', equipmentStats: { damageBonus: 4, critChance: 9, coinBonus: 16, autoClickRate: 1.5 } },
];
...
const ANCIENT_DRAGON_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'excalibur', name: 'Excalibur', icon: '⚔️', description: 'The legendary sword - balanced power for versatile warriors', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 7, critChance: 14, attackSpeed: 8, autoClickRate: 4.0 } },
  { lootId: 'ancient_dragon_armor', name: 'Ancient Dragon Plate', icon: '🛡️', description: 'Ultimate protection - ancient scales retaliate fiercely', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 25, maxHpBonus: 60, thorns: 18, autoClickRate: 2.5 } },
  { lootId: 'dragon_amulet', name: 'Dragon Amulet', icon: '💎', description: 'Grants dragon\'s power', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 6, maxHpBonus: 45, critChance: 16, autoClickRate: 3.2 } },
...
const SKELETON_WARRIOR_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'rusty_sword', name: 'Rusty Sword', icon: '⚔️', description: 'Aged but deadly', rarity: 'common', type: 'weapon', equipmentStats: { damageBonus: 2, autoClickRate: 0.5 } },
  { lootId: 'bone_armor', name: 'Bone Armor', icon: '🛡️', description: 'Assembled from skeletons', rarity: 'common', type: 'armor', equipmentStats: { defense: 7, maxHpBonus: 5, autoClickRate: 0.5 } },
  { lootId: 'bone_dagger', name: 'Bone Dagger', icon: '🗡️', description: 'Sharpened rib', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 2, critChance: 6, autoClickRate: 1.0 } },
];
...
const CURSED_SPIRIT_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'spectral_robes', name: 'Spectral Robes', icon: '🧥', description: 'Ethereal protection', rarity: 'common', type: 'armor', equipmentStats: { defense: 5, maxHpBonus: 8, autoClickRate: 0.5 } },
  { lootId: 'haunted_pendant', name: 'Haunted Pendant', icon: '📿', description: 'Whispers dark secrets', rarity: 'rare', type: 'artifact', equipmentStats: { maxHpBonus: 5, coinBonus: 8, autoClickRate: 1.0 } },
];
...
const VAMPIRE_LORD_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'blood_sword', name: 'Blood Sword', icon: '🗡️', description: 'Thirsts for blood', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 3, critChance: 8, maxHpBonus: 5, autoClickRate: 1.0 } },
  { lootId: 'vampire_cape', name: 'Vampire Cape', icon: '🧛', description: 'Grants night power - heals from damage taken', rarity: 'rare', type: 'armor', equipmentStats: { defense: 10, maxHpBonus: 12, defensiveLifesteal: 3, autoClickRate: 1.0 } },
  { lootId: 'crimson_ring', name: 'Crimson Ring', icon: '💍', description: 'Drains enemies', rarity: 'epic', type: 'artifact', equipmentStats: { critChance: 10, maxHpBonus: 8, autoClickRate: 1.5 } },
];
...
const DEATH_KNIGHT_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'necrotic_sword', name: 'Necrotic Sword', icon: '⚔️', description: 'Drains life on hit', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 4, critChance: 7, autoClickRate: 1.0 } },
  { lootId: 'death_armor', name: 'Death Knight Armor', icon: '🛡️', description: 'Forged for the undead - absorbs life and curses attackers', rarity: 'rare', type: 'armor', equipmentStats: { defense: 13, maxHpBonus: 15, defensiveLifesteal: 3, thorns: 8, autoClickRate: 1.0 } },
  { lootId: 'shadow_ring', name: 'Shadow Ring', icon: '💍', description: 'Hides in darkness', rarity: 'rare', type: 'artifact', equipmentStats: { attackSpeed: 12, coinBonus: 10, autoClickRate: 1.0 } },
];
...
const NECROMANCER_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'staff_of_souls', name: 'Staff of Souls', icon: '🪄', description: 'Commands the dead', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 5, critChance: 11, attackSpeed: 10, autoClickRate: 2.0 } },
  { lootId: 'necro_robes', name: 'Necromancer\'s Robes', icon: '🧥', description: 'Radiates dark power - siphons life force', rarity: 'epic', type: 'armor', equipmentStats: { defense: 10, maxHpBonus: 25, defensiveLifesteal: 4, autoClickRate: 1.5 } },
  { lootId: 'phylactery', name: 'Phylactery', icon: '⚱️', description: 'Stores a soul', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 30, coinBonus: 20, autoClickRate: 1.5 } },
];
...
const WRAITH_LORD_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'wraith_blade', name: 'Wraith Lord\'s Blade', icon: '⚔️', description: 'Phases through armor', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 13, attackSpeed: 10, autoClickRate: 2.0 } },
  { lootId: 'shadow_armor', name: 'Shadow Lord Armor', icon: '🛡️', description: 'Ethereal protection', rarity: 'epic', type: 'armor', equipmentStats: { defense: 14, maxHpBonus: 26, autoClickRate: 1.5 } },
  { lootId: 'wraith_crown', name: 'Crown of Shadows', icon: '👑', description: 'Rules over the dead', rarity: 'epic', type: 'artifact', equipmentStats: { maxHpBonus: 22, critChance: 10, coinBonus: 20, autoClickRate: 1.5 } },
];
...
const LICH_KING_SPECIFIC: LootItem[] = [
  ...
  { lootId: 'scarlet_dagger', name: 'Scarlet Dagger', icon: '🗡️', description: 'Pure offensive power - no defensive capabilities', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 9, critChance: 18, autoClickRate: 3.8 } },
  { lootId: 'lich_plate', name: 'Lich King\'s Plate', icon: '🛡️', description: 'Eternal protection', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 28, maxHpBonus: 70, autoClickRate: 2.5 } },
  { lootId: 'death_orb', name: 'Orb of Eternal Death', icon: '🔮', description: 'Controls life and death', rarity: 'legendary', type: 'artifact', equipmentStats: { maxHpBonus: 50, critChance: 20, coinBonus: 80, autoClickRate: 3.5 } },
...
const CRAFT_ONLY_WEAPONS: LootItem[] = [
  { lootId: 'masterwork_blade', name: 'Masterwork Blade', icon: '⚔️', description: 'Perfectly balanced steel sword', rarity: 'rare', type: 'weapon', equipmentStats: { damageBonus: 4, critChance: 10, autoClickRate: 1.0 } },
  { lootId: 'alchemists_staff', name: 'Alchemist\'s Staff', icon: '🪄', description: 'Infused with magical properties', rarity: 'epic', type: 'weapon', equipmentStats: { damageBonus: 6, critChance: 12, attackSpeed: 8, autoClickRate: 1.8 } },
  { lootId: 'vorpal_blade', name: 'Vorpal Blade', icon: '🗡️', description: 'Legendary blade that strikes true - pure glass cannon offense', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 9, critChance: 18, autoClickRate: 3.8 } },
  { lootId: 'tidal_reaver', name: 'Tidal Reaver', icon: '🔱', description: 'Legendary trident of the sea - balanced power and speed', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 8, critChance: 14, attackSpeed: 12, autoClickRate: 3.5 } },
  { lootId: 'infernal_claymore', name: 'Infernal Claymore', icon: '⚔️', description: 'Forged in volcanic fury - power with endurance', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 8, critChance: 13, maxHpBonus: 20, autoClickRate: 3.2 } },
  { lootId: 'reaper_scythe', name: 'Reaper\'s Scythe', icon: '🗡️', description: 'Harvests souls of the living - balanced offense and survivability', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 7, critChance: 16, maxHpBonus: 30, autoClickRate: 3.0 } },
  // Phase 2.5 - Advanced Legendary Items with special effects
  { lootId: 'excalibur', name: 'Excalibur', icon: '⚔️', description: 'The legendary sword of kings - strikes with divine fury (auto-attacks)', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 8, critChance: 15, autoClickRate: 4.0 } },
  { lootId: 'scarlet_dagger', name: 'Scarlet Dagger', icon: '🗡️', description: 'Crimson blade that thirsts for blood - heals with every strike', rarity: 'legendary', type: 'weapon', equipmentStats: { damageBonus: 7, critChance: 16, lifesteal: 3, autoClickRate: 3.8 } },
];

const CRAFT_ONLY_ARMOR: LootItem[] = [
  { lootId: 'reinforced_steel_plate', name: 'Reinforced Steel Plate', icon: '🛡️', description: 'Heavy spiked armor punishes attackers', rarity: 'rare', type: 'armor', equipmentStats: { defense: 15, maxHpBonus: 20, thorns: 10, autoClickRate: 1.0 } },
  { lootId: 'enchanted_silk_robes', name: 'Enchanted Silk Robes', icon: '🧥', description: 'Light armor imbued with protective magic', rarity: 'epic', type: 'armor', equipmentStats: { defense: 12, maxHpBonus: 35, attackSpeed: 10, autoClickRate: 1.5 } },
  { lootId: 'healers_plate', name: 'Healer\'s Plate', icon: '🛡️', description: 'Armor infused with restoration magic (+10% heal, 5% defensive lifesteal)', rarity: 'epic', type: 'armor', equipmentStats: { defense: 14, maxHpBonus: 30, healBonus: 10, defensiveLifesteal: 5, autoClickRate: 1.5 } },
  // NEW: Archetype-specific epic armor
  { lootId: 'assassins_leathers', name: 'Assassin\'s Leathers', icon: '🥋', description: 'Light armor designed for precision strikes - sacrifices all protection for deadly accuracy', rarity: 'epic', type: 'armor', equipmentStats: { critChance: 18, autoClickRate: 1.5 } },
  { lootId: 'battle_robes', name: 'Battle Robes', icon: '🧥', description: 'Enchanted robes offering balanced protection and utility', rarity: 'epic', type: 'armor', equipmentStats: { defense: 14, maxHpBonus: 22, attackSpeed: 8, autoClickRate: 1.5 } },
  { lootId: 'bulwark_plate', name: 'Bulwark Plate', icon: '🛡️', description: 'Impenetrable heavy armor that slows your reactions but grants unmatched protection', rarity: 'epic', type: 'armor', equipmentStats: { defense: 20, maxHpBonus: 40, attackSpeed: 12, thorns: 10, autoClickRate: 1.5 } },
  { lootId: 'berserkers_vest', name: 'Berserker\'s Vest', icon: '🎽', description: 'Ragged armor of the fearless - boosts offensive power while exposing vulnerabilities', rarity: 'epic', type: 'armor', equipmentStats: { damageBonus: 4, critChance: 15, autoClickRate: 1.5 } },
  { lootId: 'titans_armor', name: 'Titan\'s Armor', icon: '🛡️', description: 'Armor of the ancient giants - thrives on adversity', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 28, maxHpBonus: 70, defensiveLifesteal: 5, autoClickRate: 2.5 } },
  { lootId: 'abyssal_plate', name: 'Abyssal Plate', icon: '🛡️', description: 'Forged in the deepest trenches - converts pain to power', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 26, maxHpBonus: 65, attackSpeed: -5, defensiveLifesteal: 4, autoClickRate: 2.5 } },
  { lootId: 'dragonlord_armor', name: 'Dragonlord Armor', icon: '🛡️', description: 'Ultimate volcanic protection - molten scales sear attackers', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 28, maxHpBonus: 70, critChance: 5, thorns: 18, autoClickRate: 2.5 } },
  { lootId: 'dreadlord_plate', name: 'Dreadlord Plate', icon: '🛡️', description: 'Eternal undead protection - feeds on suffering and curses attackers', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 28, maxHpBonus: 70, damageBonus: 5, defensiveLifesteal: 5, thorns: 18, autoClickRate: 2.5 } },
  { lootId: 'sacred_guardian_armor', name: 'Sacred Guardian Armor', icon: '🛡️', description: 'Divine protection with regenerative power (+12% heal, 5% defensive lifesteal)', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 26, maxHpBonus: 65, healBonus: 12, defensiveLifesteal: 5, autoClickRate: 2.5 } },
  // NEW: Archetype-specific legendary armor
  { lootId: 'shadowdancer_silk', name: 'Shadowdancer Silk', icon: '🥷', description: 'Ethereal armor worn by legendary assassins - offers no protection, only deadliness', rarity: 'legendary', type: 'armor', equipmentStats: { critChance: 28, autoClickRate: 2.5 } },
  { lootId: 'fortress_plate', name: 'Fortress Plate', icon: '🛡️', description: 'Living citadel - impenetrable defense that slows monsters and reflects attacks', rarity: 'legendary', type: 'armor', equipmentStats: { defense: 30, maxHpBonus: 75, attackSpeed: 20, thorns: 20, defensiveLifesteal: 4, autoClickRate: 2.5 } },
  { lootId: 'warlords_regalia', name: 'Warlord\'s Regalia', icon: '👑', description: 'Armor of legendary commanders - balanced offense and defense for versatile warriors', rarity: 'legendary', type: 'armor', equipmentStats: { damageBonus: 5, defense: 22, maxHpBonus: 60, attackSpeed: 12, autoClickRate: 2.5 } },
];
...
const CRAFT_ONLY_ARTIFACTS: LootItem[] = [
  { lootId: 'master_craftsman_ring', name: 'Master Craftsman\'s Ring', icon: '💍', description: 'Enhances all combat abilities', rarity: 'rare', type: 'artifact', equipmentStats: { damageBonus: 3, critChance: 8, attackSpeed: 8, autoClickRate: 1.0 } },
  { lootId: 'arcane_amplifier', name: 'Arcane Amplifier', icon: '🔮', description: 'Boosts magical power', rarity: 'epic', type: 'artifact', equipmentStats: { damageBonus: 5, critChance: 15, maxHpBonus: 40, autoClickRate: 1.5 } },
  // NEW: Archetype-specific epic artifacts
  { lootId: 'assassins_mark', name: 'Assassin\'s Mark', icon: '💀', description: 'Cursed relic that grants deadly precision at the cost of all protection', rarity: 'epic', type: 'artifact', equipmentStats: { critChance: 20, autoClickRate: 1.5 } },
  { lootId: 'executioners_eye', name: 'Executioner\'s Eye', icon: '👁️', description: 'See enemy weaknesses with perfect clarity - pure offense, no protection', rarity: 'epic', type: 'artifact', equipmentStats: { damageBonus: 5, critChance: 15, autoClickRate: 1.5 } },
  { lootId: 'guardians_talisman', name: 'Guardian\'s Talisman', icon: '🛡️', description: 'Sacred relic of protectors - grants endurance and resilience, but no offensive power', rarity: 'epic', type: 'artifact', equipmentStats: { defense: 15, maxHpBonus: 50, attackSpeed: 10, autoClickRate: 1.5 } },
  { lootId: 'ironheart_stone', name: 'Ironheart Stone', icon: '💎', description: 'Grants the endurance of stone and the strength to endure any assault', rarity: 'epic', type: 'artifact', equipmentStats: { defense: 12, maxHpBonus: 45, attackSpeed: 12, defensiveLifesteal: 2, autoClickRate: 1.5 } },
  { lootId: 'adventurers_compass', name: 'Adventurer\'s Compass', icon: '🧭', description: 'Guides the way to fortune and survival - balanced power for versatile builds', rarity: 'epic', type: 'artifact', equipmentStats: { damageBonus: 3, critChance: 8, maxHpBonus: 20, coinBonus: 25, autoClickRate: 1.5 } },
  { lootId: 'infinity_amulet', name: 'Infinity Amulet', icon: '🔱', description: 'Contains limitless power - balanced might', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 8, critChance: 16, maxHpBonus: 60, coinBonus: 100, autoClickRate: 3.0 } },
  { lootId: 'maelstrom_pendant', name: 'Maelstrom Pendant', icon: '🌊', description: 'Harnesses the fury of the ocean - swift and deadly', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 7, critChance: 14, attackSpeed: 15, coinBonus: 80, autoClickRate: 3.2 } },
  { lootId: 'volcanic_heart', name: 'Volcanic Heart', icon: '🔥', description: 'Pulses with molten power - raw offensive force', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 8, critChance: 16, maxHpBonus: 60, coinBonus: 70, autoClickRate: 3.0 } },
  { lootId: 'crown_of_eternity', name: 'Crown of Eternity', icon: '👑', description: 'Rules over life and death - enduring power', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 6, critChance: 18, maxHpBonus: 70, coinBonus: 120, autoClickRate: 3.5 } },
  // NEW: Archetype-specific legendary artifacts
  { lootId: 'heart_of_reaper', name: 'Heart of the Reaper', icon: '💀', description: 'Ultimate offensive artifact - Death incarnate, offering no mercy or protection', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 8, critChance: 20, autoClickRate: 3.8 } },
  { lootId: 'perfect_prism', name: 'Perfect Prism', icon: '🔷', description: 'Focuses all power into critical strikes - fragile but devastating', rarity: 'legendary', type: 'artifact', equipmentStats: { critChance: 25, autoClickRate: 3.5 } },
  { lootId: 'aegis_of_titans', name: 'Aegis of Titans', icon: '🛡️', description: 'Legendary shield of the ancient giants - unbreakable defense with no offensive power', rarity: 'legendary', type: 'artifact', equipmentStats: { defense: 25, maxHpBonus: 100, attackSpeed: 20, defensiveLifesteal: 3, autoClickRate: 2.5 } },
  { lootId: 'bastion_core', name: 'Bastion Core', icon: '⚙️', description: 'Mechanical heart of an ancient guardian - reflects damage while enduring endless assault', rarity: 'legendary', type: 'artifact', equipmentStats: { defense: 20, maxHpBonus: 70, attackSpeed: 18, thorns: 20, autoClickRate: 2.5 } },
  { lootId: 'equilibrium_orb', name: 'Equilibrium Orb', icon: '☯️', description: 'Perfect balance of offense and defense - the way of the warrior sage', rarity: 'legendary', type: 'artifact', equipmentStats: { damageBonus: 6, critChance: 15, defense: 12, maxHpBonus: 60, attackSpeed: 10, autoClickRate: 3.2 } },
];
