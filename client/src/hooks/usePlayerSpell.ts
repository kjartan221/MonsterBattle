...
export function usePlayerSpell(): UsePlayerSpellResult {
  ...
  const loadEquippedSpell = useCallback(async () => {
    try {
      ...
      const inventoryResponse = await apiFetch('/api/inventory/get');
  ...
  const castSpell = useCallback(async (): Promise<{
    ...
    try {
      ...
      const response = await apiFetch('/api/spells/cast', {
        method: 'POST',
  ...
  const equipSpell = useCallback(async (inventoryId: string): Promise<boolean> => {
    try {
      const response = await apiFetch('/api/spells/equip', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const unequipSpell = useCallback(async (): Promise<boolean> => {
    try {
      const response = await apiFetch('/api/spells/unequip', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      });
