...
export default function SellItemModal({ wallet, onClose, onSuccess }: SellItemModalProps) {
  ...
  const loadSellableItems = async () => {
    try {
      ...
      const response = await apiFetch('/api/inventory/get?mintedOnly=true&excludeListed=true');
  ...
  const handleListItem = async () => {
    ...
    try {
      ...
      const { publicKey: serverIdentityKey } = await (await apiFetch('/api/server-identity-key')).json();
      ...
      const response = await apiFetchStepUp('/api/marketplace/list-item', {
        wallet,
        context: 'list',
        body: {
